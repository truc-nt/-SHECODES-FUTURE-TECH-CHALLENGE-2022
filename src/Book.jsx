...
import './assets/css/Page18-19.css'

import {LargePapers, SmallPapers} from './components/Paper'
...
const Book = () => {
  ...
  const addToRefs = (el) => {
    if (el && !ref.current.includes(el)) {
      ref.current.push(el)
      w = ref.current[0].clientWidth
      
    }
  }

  const handlePrevBtn = () => {
    if (ref.current.length !== 0 && currentLocation > 0) {
    ...
    } else if (ref.current.length === 0 && currentIndex > 0) setCurrentIndex(currentIndex - 1)
    
  }

  const handleNextBtn =  () => {
    if (ref.current.length !== 0 && currentLocation < maxLocation - 1) {
      setCurrentLocation(currentLocation + 1)
      ref.current[currentLocation].classList.add('flipped')
      ref.current[currentLocation].style.zIndex = currentLocation + 1
    } else if (ref.current.length === 0 && currentIndex !== maxLocation * 2 - 3) {
